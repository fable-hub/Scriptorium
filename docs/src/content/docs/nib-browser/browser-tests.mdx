...
`Fable.Nib.Browser` provides test helpers that integrate with `Fable.Quill`'s test DSL. They automatically create a headless Chromium browser, run the body, then close the browser and page.
...
`dtestPage` marks the test as `ftest` (focused). On CI, `Fable.Quill` will detect focused tests and exit with code 1 — preventing the build from passing.
:::
