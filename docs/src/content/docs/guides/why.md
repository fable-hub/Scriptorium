...
Fable Scriptorium is built to change that. It provides a family of focused libraries for testing Fable applications, all written in F#, designed to run on any Fable-supported platform with a consistent experience — the same API whether you target Node.js, the browser, or plain .NET.
...
|---|---|
| `Fable.Ink` | Colour and style for terminal output |
| `Fable.Parchment` | Structured logging that dips into Ink |
| `Fable.Nib` | Composable assertions, no runner dependency |
| `Fable.Quill` | Test runner that holds Nib, Ink, and Parchment together |
| `Fable.Nib.Snapshot` | Snapshot testing on top of Nib |
| `Fable.Nib.Browser` | Playwright-powered DOM assertions |

## Design principles

### Every failure is collected before reporting

`Fable.Nib` never throws on the first failing assertion. An assertion chain continues to the end, collecting every failure. Only when `assertThat` returns does it throw — with a single exception carrying every message.
...
- Use `Fable.Nib` assertions inside Expecto, Jest, or any other test framework.
- Use `Fable.Ink` to colour any string output in an application.
- Use `Fable.Parchment` for structured logging without any test dependency.

Nothing forces you to use the full stack. Start with what you need.

### Respected conventions

- `NO_COLOR` — `Fable.Ink` honours the [no-color.org](https://no-color.org) convention. Set `NO_COLOR=1` to strip all ANSI codes from output.
- `CI` — `Fable.Quill` detects the `CI` environment variable and refuses to exit with code 0 if focused tests (`ftest`) are committed to the repository.
- `UPDATE_SNAPSHOTS` — `Fable.Nib.Snapshot` uses this variable to trigger snapshot updates.
