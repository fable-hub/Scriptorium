...
A configurer on a list propagates to all children (see [Configuration](/Fable.Scriptorium/quill/configuration)):
...
Fable.Quill detects when tests run in CI and will throw an error if any tests are focused.
:::
